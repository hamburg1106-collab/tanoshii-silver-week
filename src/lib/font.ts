 ...
 * @font-face の定義を並べたCSSだけで350KBある。
 * iPhoneには「ヒラギノ丸ゴ」が標準で入っていて不要なので、
 * 無条件に読むとiPhone側がまるごと無駄打ちになる。
 *
 * パークの電波で350KBを待たされるのは避けたい。
 */
...
function hasLocalFont(name: string): boolean {
...
export function loadRoundedFontIfMissing(): void {
  try {
